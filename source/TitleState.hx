...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		credTextShit = new KadeAlphabet(0, 0, "", true);
	...
	function createCoolText(textArray:Array<String>, ?offset:Float = 0)
	{
		...
		{
			var money:KadeAlphabet = new KadeAlphabet(0, 0, textArray[i], true, false);
			if (textArray[i].length > 20)
			{
				money.resizeText(0.75,0.75);
			}
			if (textArray[i].length > 35)
			{
				money.resizeText(0.5,0.5);
			}
	...
	{
		if(textGroup != null) {
			var coolText:KadeAlphabet = new KadeAlphabet(0, 0, text, true, false);
			if (text.length > 20)
			{
				coolText.resizeText(0.75,0.75);
			}
			if (text.length > 35)
			{
				coolText.resizeText(0.5,0.5);
			}
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				createCoolText([curWacky[0]]);
					...
					// credTextShit.text += '\nNight';
					case 31:
						addMoreText('Eye'); // credTextShit.text += '\nFunkin';
